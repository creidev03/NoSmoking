...
describe("completeOnboarding", () => {
  ...
  it("redirects to /dashboard after successful completion", async () => {
    ...
    await expect(completeOnboarding("es")).rejects.toThrow("NEXT_REDIRECT");
    expect(mockRedirect).toHaveBeenCalledWith("/es/dashboard");
  });

  it("throws when no onboarding record found", async () => {
    await expect(completeOnboarding("es")).rejects.toThrow();
  });
});
